...
import { getExperience, addExperience, updateExperience, deleteExperience } from "../../../../lib/adminStore"

const EMPTY = { title: "", company: "", location: "", duration: "", responsibilities: "" }
...
export default function ExperienceAdmin() {
  ...
  const handleSave = e => {
    ...
    if (editing) setItems(updateExperience(editing, payload))
    else setItems(addExperience(payload))
    closeForm()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-normal text-stone-100 mb-1">Experience</h1>
      ...
      <div className="border border-stone-900 bg-[#0d0b09] divide-y divide-stone-900">
        {items.length === 0 && <p className="px-6 py-8 text-sm text-stone-700 text-center">No experience yet.</p>}
        {items.map(item => (
          <div key={item.id} className="flex items-start gap-4 px-5 py-4 hover:bg-white/2 group">
            <div className="flex-1 min-w-0">
              <p className="text-sm text-stone-300 mb-0.5">{item.title}</p>
              <p className="text-[11px] text-stone-500">{item.company} · {item.location}</p>
              <p className="text-[10px] text-stone-700 mt-0.5">{item.duration}</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative z-10 w-full max-w-lg bg-[#0d0b09] border-l border-stone-900 h-full overflow-y-auto p-7 flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-normal text-stone-200">{editing ? "Edit Role" : "Add Role"}</h2>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4">
              {[
                { label: "Job Title",  key: "title",    placeholder: "UI Engineer" },
                { label: "Company",    key: "company",  placeholder: "Banyan Cloud Inc." },
                { label: "Location",   key: "location", placeholder: "San Jose, CA" },
                { label: "Duration",   key: "duration", placeholder: "May 2021 - May 2022" },
              ...
              <div>
                <label className="block text-[10px] tracking-widest uppercase text-stone-600 mb-1.5">
                  Responsibilities <span className="normal-case text-stone-700">(one per line)</span>
                </label>
                <textarea value={form.responsibilities} onChange={e => f("responsibilities", e.target.value)} rows={6}
                  placeholder={"Developed reusable UI components\nConducted unit testing\n…"}
      ...
      {confirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative z-10 bg-[#0d0b09] border border-stone-800 p-7 max-w-sm w-full">
            <p className="text-sm text-stone-300 mb-1">Delete this role?</p>
            <p className="text-[11px] text-stone-600 mb-6">This action cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => { setItems(deleteExperience(confirm)); setConfirm(null) }} className="flex-1 bg-red-600/80 text-white text-[11px] tracking-[0.15em] uppercase py-2.5 hover:bg-red-600 transition-colors">Delete</button>
